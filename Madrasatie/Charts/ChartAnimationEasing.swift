...
internal struct EasingFunctions
{
    ...
    internal static let EaseOutBack = { (elapsed: TimeInterval, duration: TimeInterval) -> Double in
        ...
        let positionMinusOne: TimeInterval = position - 1.0
        let a: Double = positionMinusOne * positionMinusOne
        let b: Double = (s + 1.0) * positionMinusOne
        let c: Double = s * positionMinusOne
        let d: Double = a * (b + s)
        let result: Double = d + 1.0
        return result
    }
