...
protocol VirtualClassroomViewControllerDelegate {
    func virtualPressed(calendarType: CalendarStyle?)
}

class VirtualClassroomViewController: UIViewController, FSCalendarDelegate, FSCalendarDataSource {
    
    @IBOutlet weak var virtualClassroomTableView: UITableView!
    var virtualDelegate: VirtualClassroomViewControllerDelegate?
    var user: User!
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        print("viewdidloadd")
        virtualClassroomTableView.dataSource = self
        virtualClassroomTableView.delegate = self
        calendarStyle = .week
         tempCalendarStyle = .week
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.4) {
            if let calendarView = self.virtualClassroomTableView.viewWithTag(4) as? FSCalendar{
                let calendarHeight = calendarView.constraints.filter({$0.identifier == "calendarHeight"}).first
                let monthLabel = self.virtualClassroomTableView.viewWithTag(3) as! UILabel
                ...
                self.virtualClassroomTableView.reloadData()
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        virtualDelegate?.virtualPressed(calendarType: self.calendarStyle)
        
    }
    /// Description
       /// - Show next month/week dates inside FSCalendar.
       @objc func calendarNextButtonPressed(sender: UIButton){
        guard let calendarView = self.virtualClassroomTableView.viewWithTag(4) as? FSCalendar else{
               return
           }
...
extension VirtualClassroomViewController: SectionVCToVirtualClassroomDelegate{
...
extension VirtualClassroomViewController: IndicatorInfoProvider{
    
    func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
        return IndicatorInfo(title: "Virtual Classroom", counter: "", image: UIImage(named: "virtualIcon"),
                             backgroundViewColor: App.hexStringToUIColorCst(hex: "#dd6b67", alpha: 1.0), id: App.virtualClassroomId)
    }
    
}
extension VirtualClassroomViewController:  UITableViewDataSource{
...
extension VirtualClassroomViewController: UITableViewDelegate{
